...
export default function PaymentMethod({ paymentMethod }) {
    const [name, nameSet] = React.useState(paymentMethod ? paymentMethod.name : "");
    const { data, setData, post, processing, errors, reset } = useForm({
        name: paymentMethod ? paymentMethod?.name : "",
        slug: paymentMethod ? paymentMethod?.slug : "",
    ...
    const submit = (e) => {
        ...
        if (paymentMethod) {
            post(route("admin.paymentMethods.update", { id: paymentMethod.id }));
        } else {
            post(route("admin.paymentMethods.store"));
        }
    };

    React.useEffect(() => {
        setData("name", name);
        if (!paymentMethod) setData("slug", textToSlug(name));
    }, [name]);
    ...
    return (
        <TwoColumnLayout>
            ...
            <TwoColumnLayout.Heading>
                <PageHeading>
                    <PageHeading.Title>
                        {/* paymentMethod - {paymentMethod ? "Edit " + paymentMethod.name : "Create"} */}
                        {paymentMethod ? (
                    ...
                    <PageHeading.Actions>
                        <Button asChild variant="outline">
                            <Link href={route("admin.paymentMethods.index")}>
                                Cancel
                            </Link>
                        </Button>
                    ...
                    <div>
                        {paymentMethod ? (
                            <Link className="text-blue-600 italic text-sm">
                                {route("admin.paymentMethods.edit", paymentMethod.id)}
                            </Link>
                        ) : (
                            ""
                        )}
            ...
            <TwoColumnLayout.Content>
                ...
                <TwoColumnLayout.Aside>
                    {paymentMethod && (
                        <ShadcnCard title={paymentMethod.name}>
                            <TextMuted className="inline-block pt-2">
                                Created at
                            </TextMuted>
                            <TextLarge className={`leading-[0]`}>
                                {paymentMethod.created_at_string}
                            </TextLarge>
                            <TextMuted className="inline-block pt-2">
                                Last Updated
                            </TextMuted>
                            <TextLarge className={`leading-[0]`}>
                                {paymentMethod.updated_at_string}
                            </TextLarge>
                        </ShadcnCard>
                    )}
